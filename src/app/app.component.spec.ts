...
describe("AppComponent", () => {
  ...
  beforeEach(async () => {
    ...
    mockPoetryService = {
      BASE_URL: "mymockurl/",
      getAllFromAuthorAndTitle: jasmine
        .createSpy("getAllFromAuthorAndTitle")
        .and.returnValue(Promise.resolve(mockData)),
      getAllFromTitle: jasmine
        .createSpy("getAllFromTitle")
        .and.returnValue(Promise.resolve(mockData)),
      getAllFromAuthor: jasmine
        .createSpy("getAllFromAuthor")
        .and.returnValue(Promise.resolve(mockData)),
    };
  ...
  describe("Poem Search", () => {
    ...
    it("should pop up an error if the poetry call was unsuccessful", async () => {
      mockPoetryService.getAllFromAuthorAndTitle = jasmine
        .createSpy("getAll")
        .and.returnValue(Promise.reject("mock error"));
      spyOn(Swal, "fire");
      await app.search(searchAT);
    ...
    it("should allow a poem search with only title", async () => {
      ...
      expect(mockPoetryService.getAllFromTitle).toHaveBeenCalledOnceWith(
        searchT.title,
      );
    });
    ...
    it("should allow a poem search with only author", async () => {
      ...
      expect(mockPoetryService.getAllFromAuthor).toHaveBeenCalledOnceWith(
        searchA.author,
      );
    });
    ...
    it("should allow a poem search with both title and author", async () => {
      await app.search(searchAT);
      expect(
        mockPoetryService.getAllFromAuthorAndTitle,
      ).toHaveBeenCalledOnceWith(searchAT.author, searchAT.title);
    });
  });
